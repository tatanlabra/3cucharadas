# Multiagentes III — mensajes de difusión

Estado: **preparado, no publicado.** La publicación en vivo requiere confirmación humana explícita.

Campaña UTM: `multiagente-memoria-gobernada`

## Mastodon

### ES root

La memoria del sistema ya recuperaba bien. El problema pasó a ser otro: qué persiste, qué caduca y qué nunca debe salir de su ámbito.

Tercera parte: cuatro fuentes separadas por contrato, no fusionadas.

Varias mejoras de recuperación probadas, una promovida. Y una compuerta que publico en rojo a propósito.

#RAG #AIAgents #KnowledgeGraph https://3cucharadas.cl/ia/productividad/desarrollo/multiagente-memoria-gobernada-poc/?utm_source=mastodon&utm_medium=social&utm_campaign=multiagente-penta-agent-memoria-gobernada-poc&utm_content=es

### EN reply

The system already retrieved well. The harder question came next: what should persist, what should expire, and what must never leave its scope.

Part three: four sources kept apart by contract, not merged.

Several retrieval improvements tested, one promoted. And a gate I am publishing red on purpose.

#RAG #AIAgents #KnowledgeGraph https://3cucharadas.cl/en/ia/productividad/desarrollo/multiagente-memoria-gobernada-poc/?utm_source=mastodon&utm_medium=social&utm_campaign=multiagente-penta-agent-memoria-gobernada-poc&utm_content=en

## Bluesky

### ES root

Ya recuperaba bien. La pregunta pasó a ser qué persiste, qué caduca y qué nunca sale de su ámbito.

Varias mejoras probadas, una promovida. El reordenador ordenaba mejor y aun así lo rechacé: el tiempo de respuesta subía demasiado.

https://3cucharadas.cl/ia/productividad/desarrollo/multiagente-memoria-gobernada-poc/?utm_source=bluesky&utm_medium=social&utm_campaign=multiagente-penta-agent-memoria-gobernada-poc&utm_content=es

### ES reply

Publico con una compuerta en rojo: el corpus creció de 796 a 817 documentos y el índice no se regeneró.

La instantánea es fiel a su fecha. Lo que caducó es la vigencia, no el dato. Una compuerta verde mientras el corpus crece es peor que no tenerla.

### EN root

Retrieval was already fine. The question became what should persist, what should expire, and what must never leave its scope.

Several improvements tested, one promoted. The reranker ranked better and I rejected it anyway: response time grew too much.

https://3cucharadas.cl/en/ia/productividad/desarrollo/multiagente-memoria-gobernada-poc/?utm_source=bluesky&utm_medium=social&utm_campaign=multiagente-penta-agent-memoria-gobernada-poc&utm_content=en

### EN reply

I am publishing with a gate in the red: the corpus grew from 796 to 817 documents and the index was not rebuilt.

The snapshot is still faithful to its date. What expired is its currency, not the data. A gate that stays green while the corpus grows is worse than no gate.
