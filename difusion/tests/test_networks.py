...
def test_bluesky_richtext_facets_include_mentions_and_hashtags():
    from atproto import models

    text = "Probé @qdrant.bsky.social con búsqueda híbrida. #RAG"
    facets = BlueskyClient._richtext_facets(
        text,
        models,
        lambda handle: {"did": "did:plc:t7zhu52aj6qsxenmve63o4xj"}
        if handle == "qdrant.bsky.social"
        else {},
    )
    ...
    mention, tag = facets
    encoded = text.encode("utf-8")
    assert encoded[mention.index.byte_start : mention.index.byte_end].decode("utf-8") == "@qdrant.bsky.social"
    assert mention.features[0].did == "did:plc:t7zhu52aj6qsxenmve63o4xj"
    assert encoded[tag.index.byte_start : tag.index.byte_end].decode("utf-8") == "#RAG"
