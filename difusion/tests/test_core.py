...
def test_destinations_are_content_gated(repo):
    ...
    assert rows["dev"]["status"] == "listo"
    assert rows["medium"]["status"] == "listo"
    assert rows["juliabloggers"]["status"] == "listo"
    assert rows["r-bloggers"]["status"] == "bloqueado"


def test_destinations_declaran_idioma_y_publico(repo):
    rows = {row["destination"]: row for row in destination_status(repo, "casen2024-julia-waffles")}
    assert rows["mastodon-es"]["lang"] == "es"
    assert rows["planet-python"]["lang"] == "en"
    assert "politica-publica" in rows["linkedin-es"]["audiencia"]
    # Un destino sin reglas de envio confirmadas nunca se declara listo.
    assert rows["comunidades-datos-gis-es"]["status"] == "pendiente-verificar"


def test_post_sin_version_en_igual_alcanza_destinos_es(repo, tmp_path):
    """Antes esto lanzaba PostError y dejaba el articulo sin evaluar en ningun canal."""
    solo_es = [
        p for p in (repo / "_posts").glob("*.md") if not p.name.endswith("-en.md")
    ]
    assert solo_es, "el fixture necesita al menos un post ES"
    rows = {row["destination"]: row for row in destination_status(repo, "casen2024-julia-waffles")}
    assert rows["mastodon-es"]["status"] == "listo"
