...
class BlueskyClient:
    ...
    def _did(value: Any) -> str:
        ...
        if isinstance(value, dict):
            return str(value.get("did", ""))
        return str(getattr(value, "did", ""))

    @classmethod
    def _richtext_facets(
        cls,
        text: str,
        models: Any,
        ...
        for match in cls.HANDLE_RE.finditer(text):
            ...
            did = cls._did(resolve_handle(handle))
            if not did:
                raise PublishError(f"No se pudo resolver el handle Bluesky @{handle}")
            byte_start = len(text[: match.start()].encode("utf-8"))
            byte_end = len(text[: match.end()].encode("utf-8"))
            facets.append(
                models.AppBskyRichtextFacet.Main(
    ...
    def publish_thread(
        ...
        try:
            root = client.send_post(
                text=es.text,
                langs=["es"],
                facets=self._richtext_facets(es.text, models, client.resolve_handle),
        ...
        try:
            reply = client.send_post(
                text=en.text,
                langs=["en"],
                facets=self._richtext_facets(en.text, models, client.resolve_handle),
    ...
    def resume_thread(
        ...
        try:
            reply = client.send_post(
                text=message.text,
                langs=["en"],
                facets=self._richtext_facets(message.text, models, client.resolve_handle),
                embed=self._external_embed(client, models, draft, "en"),
                reply_to=reply_ref,
            )
        except Exception as exc:
