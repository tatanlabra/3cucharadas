...
def cmd_destinations(args: argparse.Namespace) -> int:
    ...
    elif args.destination_command == "record":
        salida = registrar_resolucion(repo, args.ref)
        print(destination_checklist(repo, args.ref), end="")
        print(f"\nResolucion registrada en {salida.relative_to(repo)}")
    else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    record = dest_commands.add_parser(
        "record", help="Resuelve destinos y deja la traza en difusion/state/destinos/"
    )
    record.add_argument("ref")
    record.set_defaults(func=cmd_destinations)
    return parser
