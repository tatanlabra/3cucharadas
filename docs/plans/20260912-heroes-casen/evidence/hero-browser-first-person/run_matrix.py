...
for width in [390,1440]:
 ...
 for post in POSTS:
  for theme in ['academic-day','academic-night']:
   ...
   evaluate('(async()=>{await document.fonts.ready;await new Promise(r=>setTimeout(r,600))})()')
   anchor='#'+f['ref'];sel='details[id='+json.dumps(f['ref'])+']'
   ab('focus',sel+' summary')
   policy_before=evaluate('document.querySelector('+json.dumps(sel)+').open')
   ab('press','Enter')
   policy_toggled=evaluate('document.querySelector('+json.dumps(sel)+').open')
   if policy_toggled==policy_before:fail.append('policy keyboard toggle failed')
   if not policy_toggled:ab('press','Enter')
