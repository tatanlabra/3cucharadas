...
const HERE=path.dirname(new URL(import.meta.url).pathname),PERF=path.resolve(HERE,'..'),PLAN=path.resolve(PERF,'../..'),REPO=path.resolve(PLAN,'../../..');
const pairs=1,OUT=path.resolve(process.argv[3]||path.join(PERF,'presentation-diagnosis/product-canary'));
...
const ROOTS={baseline:'/tmp/3c-heroes-first-person-site',candidate:'/tmp/3c-heroes-first-person-site'},PORTS={baseline:4041,candidate:4042};
...
const prototype=fs.readFileSync('/tmp/lunr-en-deferred-canary.js','utf8');
const searchChecks=[];
...
try{
 ...
 for(let pair=1;pair<=pairs;pair++)for(const [index,[page,url]] of Object.entries(Object.entries(PAGES))){
  ...
  for(const side of order){
   ...
   await context.route('**/*',route=>{const u=new URL(route.request().url());if(side==='candidate'&&u.hostname==='127.0.0.1'&&u.pathname==='/assets/js/lunr/lunr-en.js')return route.fulfill({status:200,contentType:'text/javascript',headers:{'Cache-Control':'no-store'},body:prototype});return ['127.0.0.1','localhost'].includes(u.hostname)?route.continue():route.abort('blockedbyclient');});
   ...
   for(const temperature of ['cold','warm']){
    ...
    try{
     ...
     row.resources_sha256={};for(const res of raw.resources){const u=new URL(res.name);if(u.hostname==='127.0.0.1'){const f=path.join(ROOTS[side],decodeURIComponent(u.pathname));if(fs.existsSync(f)&&fs.statSync(f).isFile())row.resources_sha256[u.pathname]=(side==='candidate'&&u.pathname==='/assets/js/lunr/lunr-en.js')?sha('/tmp/lunr-en-deferred-canary.js'):sha(f);}}
   ...
   const searchCheck=await tab.evaluate(()=>{const before=Boolean(window.idx);const input=document.querySelector('input#search');if(!input)return {error:'input absent'};input.value='casen';input.dispatchEvent(new Event('focus'));input.dispatchEvent(new Event('input'));input.dispatchEvent(new Event('keyup'));return {beforeIndexBuilt:before,afterIndexBuilt:Boolean(window.idx),resultCount:document.querySelectorAll('#results article').length,text:document.querySelector('#results')?.textContent.slice(0,120),buildStart:window.__LUNR_BUILD_START__??null,buildEnd:window.__LUNR_BUILD_END__??null,note:'Synthetic DOM events after measurement; does not validate visual focus accessibility'}});searchChecks.push({page,side,...searchCheck});dump('search-checks.json',searchChecks);await context.close();await activeBrowser.close();activeBrowser=null;closed++;
  }
 }
