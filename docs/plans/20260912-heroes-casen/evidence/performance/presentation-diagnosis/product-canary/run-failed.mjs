...
try{
 ...
 for(let pair=1;pair<=pairs;pair++)for(const [index,[page,url]] of Object.entries(Object.entries(PAGES))){
  ...
  for(const side of order){
   ...
   await context.addInitScript({content:init});const tab=await context.newPage();
   for(const temperature of ['cold','warm']){
    ...
    row.cpu_during_navigation=delta(before,cpu());rows.push(row);dump('runs.json',rows);console.log(JSON.stringify({pair,page,side,temperature,metrics:row.metrics,missing:row.missing,error:row.error}));
   }
